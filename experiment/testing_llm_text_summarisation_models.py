...
model_1 = "llama3.1:8b"
model_2 = "granite4:tiny-h"

prompt_1 = """
Can you please summarise the following case study from Digital Catapult, UK
in 150 words. 

- No need to include the title.
- Include Catapult support, the outcome and impact of the project if described. 
- Include government departments if in the case study.
- Include company name if mentioned.
- Include region and international information if mentioned.
...
models:list[str] = [model_1,model_2]
prompts:list[str] = [prompt_1, prompt_2, prompt_3]
...
for i,prompt in enumerate(prompts,1):
    for model in models:
        ...
        with open("experiment/outputs.txt", 'a') as file:
            response = str(response.response)
            response_edited = add_newlines_after_period(response)
            first_line = f"Reponse from {model} for prompt_{i}"
